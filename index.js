...
async function selectTimeRange(page, account, timeRange) {
  log.info('Selecting time range for', account.type, ':', timeRange.from, '-', timeRange.to);
...
class DkbScraper {
  ...
  async scrape(accounts, from, to) {
    ...
    await createScreenshot(page, path.join(this.options.screenshotDir, 'before_login.png'));
    ...
    const accountsToScrape =
      accounts[0] === 'all'
        ...
        : allAccounts.filter(
            allAccount =>
              accounts.filter(account => allAccount.name.indexOf(account) !== -1).length > 0
          );
    ...
    await createScreenshot(page, path.join(this.options.screenshotDir, 'after_logout.png'));
